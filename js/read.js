...
infinity, nan – специальные занчения типа number
...
Каждый раз, когда начинается новая итерация внешнего цикла
внутренний цикл for (let j = 2; j < i; j++) запускается заново,
и переменная j создаётся заново со значением 2.

🔍 Почему так происходит:

В JavaScript переменные, объявленные через let внутри цикла for,
живут только в пределах этого цикла (внутренней области видимости).
...
Интерфейс окна source состоит из трёх зон:
Вкладка source
...
Это удобно, когда нужно остановиться только при определённом значении переменной или для определённых параметров функции.
...
Можно нажать на + и ввести выражение. Отладчик покажет его значение, автоматически пересчитывая его в процессе выполнения.
...
Если нажать на элемент стека (например, «anonymous»), отладчик перейдёт к соответствующему коду, и нам представляется возможность его проанализировать.
...
В Global перечисляются глобальные переменные (вне каких-либо функций).
